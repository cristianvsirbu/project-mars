  ...
  return (
    <div className="navbar select-none">
      ...
      <nav className="mx-6 h-20 flex lg:justify-evenly md:mx-16 lg:mx-32 md:h-14 lg:h-16">
        {/* Logo */}
        <div className="h-full lg:mt-3 md:w-1/4 flex items-center justify-center">
          <Link to="/" aria-label="Go to Home">
            <img src="/assets/logos/logo.webp" className="w-auto h-14 lg:h-16" alt="Logo" />
          </Link>
        </div>
        ...
        <div className="flex flex-1 justify-end items-center md:hidden">
          {/* Close Menu */}
          {menuIsOpen && (
            <button className="h-8 w-8" onClick={() => setMenuIsOpen(false)}>
          ...
          {/* Open Menu */}
          {!menuIsOpen && (
            <button className="h-8 w-8" onClick={() => setMenuIsOpen(true)}>
          ...
          <div
            ...
            } p-6 top-20 flex-col absolute rounded-xl min-w-[15rem] bg-slate-900 sidebar`}
            ref={dropdownRef}
          >
            {menuIsOpen && (
              <ul className="flex flex-col items-center">
                {navLinks.map((navLink) => (
                  <li
                    key={navLink.label}
                    className="text-white blink cursor-pointer font-bold py-2 md:py-0 text-3xl md:text-[1.2rem] xl:text-[1.5rem]"
                  >
                    <NavLink to={navLink.path}>{navLink.label}</NavLink>
                  </li>
                ))}
