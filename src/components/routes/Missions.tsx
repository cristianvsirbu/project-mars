...
const MissionsPage = () => {
	...
	const launchesByDecade = missionsModel
		? (missionsModel as any).launches_by_decade
		: [];
	...
	const Missions = () => {
		const missionsModel = data.find((model) => model.category === "missions");
		const missions: MissionType[] = missionsModel
			? (missionsModel as any).missions || []
			: [];
		...
		const getColorClass = (value: string) => {
			if (value.toLowerCase().includes("fail")) {
				...
				value.toLowerCase().includes("mostly") ||
			...
			<div className="mx-8 grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 4k:grid-cols-6 text-white">
				{missions.map((mission, index) => (
					<div key={index} className="blur__card p-10 parallax">
						...
						<ul
							key={index}
							className="text-[1.1rem] lg:text-[1.3rem] 4k:text-[1.4rem]"
						>
							{Object.entries(mission).map(([key, value]) => {
								...
								return (
									<li
										key={key}
										className={`flex flex-col text-slate-400 font-medium`}
									>
