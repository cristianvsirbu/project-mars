	...
	const navLinks = routes.flatMap((route) => {
		if (route.children) {
			return route.children.map((childRoute) => (
				<li
					className={`text-white blink  cursor-pointer font-bold py-2 md:py-0 text-[32px] md:text-[20px] lg:text-[24px] xl:text-[32px] 4k:text-[46px]`}
					key={childRoute.path}
		...
		<div className="navbar select-none">
			...
			<nav className="mx-6 h-24 flex justify-between md:mx-16 md:h-14 lg:h-16 xl:h-20 2xl:h-24">
				...
				<div className="flex flex-1 justify-end items-center md:hidden">
					...
					>
						{menuIsOpen && (
							<ul className="flex flex-col  items-center">{navLinks}</ul>
						)}
					</div>
				</div>
			</nav>
